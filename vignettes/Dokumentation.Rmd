...
Der Schätzer folgt der Idee ein Signal in sinusartige Funktionen mit unkorrelierten Koeffizienten zu zerlegen. 
$$
x_t=\sum \limits_{j \in F_n}a_j [\cos(\omega_jt) + i \sin(\omega_jt)]
$$
Die "unwichtigen" Bestandteile werden entfernt. Ziel ist die bestmögliche Approximation der spektralen Leistungsdichte.
...
$$
mit  
$\omega_j = 2 \pi j$ (den Fourier Frequenzen der Zeitreihe $\{x_1,...,x_n\}$) und $j \in F_n=\{j \in \mathbb{Z} : -\pi < \omega_j \leq \pi \}$
...
```
Da für die Fourier-Frequenzen gilt $I(-\omega_j) = I(\omega_j)$ bietet es sich an nur eine Seite des Periodogramms in die Grafik aufzunehemen.

Wenn eine Zeitreihe glatt (bzw. verwackelt) erscheint, dann werden die Werte des Periodogramms für niedrige (bzw. hohe) Frequenzen im Verhältnis zu seinen anderen Werten groß sein, und man sagt, dass der Datensatz einen Überschuss an niedrigen (bzw. hohen) Frequenz aufweist. 
Bei einer rein zufälligen Serie sollten alle Sinus- und Cosinuskurven von gleicher Bedeutung sein, und daher wird das Periodogramm zufällig um eine Konstante variieren.
Sollte eine Zeitreihe ein starkes sinusartiges Signal für eine Frequenz aufweise, dann wird das Periodogramm eine Spitze bei dieser Frequenz aufweisen.
Falls die Zeitreihe ein starkes nicht-sinusbasiertes Signal für eine Frequenz aufweißt, wird das Periodogramm eine Spitze in dieser Frequenz und den Vielfachen aufweißen.
...
```{r}
set.seed(1234)
ma_obj <- arma_sim(theta = c(0.6,-0.2), n = 1000, burnin = 2000)
...
## Quellen
Brockwell, Peter J., Richard A. Davis, and Matthew V. Calder. Introduction to time series and forecasting. Vol. 2. New York: springer, 2002.
>>>>>>> 466ad5bad890c669a21d2df3b884d3df8ef59149
