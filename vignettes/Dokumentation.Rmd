...
$$
mit$\omega_j = \frac{2 \pi j}{n}$, den Fourier Frequenzen der Zeitreihe $\{x_1,...,x_n\}$, und $j \in F_n=\{j \in \mathbb{Z} : -\pi < \omega_j \leq \pi \}$. Man beachte, dass $F_n$ n Elemente enthält. Die Folge $\{a_j: j\in F_n\}$ wird als diskrete Fourier-Transformation der Zeitreihe $\{x_1,...,x_n\}$ bezeichnet.
$$
