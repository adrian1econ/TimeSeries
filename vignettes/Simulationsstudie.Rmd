...
In diesem Abschnitt soll untersucht werden ob der Durbin-Levinson Algorithmus gute Vorhersagen für simulierte Daten liefert. Dafür werden zum einen 200 Datenpunkte verscheidener Prozesse simuliert. Nach 50 Beobachtungen wird eine "expanding window out-of-sample" Vorhersage durchgeführt. Also es werden alle Daten bis zum Zeitpunkt *t* verwendet um die Realisation in *t+1* vorherzusagen. Danach wird der Datensatz für die nächste Vorhersage um die Realisation in *t+1* erweitert. Für diese Art von Forecast wird eine Funktion definiert.

```{r}
dl_forecast <- function(x){
        
        x <- x$arma
        forecast <- numeric(200)
...
Im folgenden werden beispielhaft Daten zu folgenden Prozessen simuliert: AR(1), AR(2), AR(3), MA(1), ARMA(1,1).

```{r}
set.seed(4637)

dl_sim1 <- arma_sim(phi = 0.8, n=200, burnin=1000)
dl_sim2 <- arma_sim(phi = 0.3, n=200, burnin=1000)
dl_sim3 <- arma_sim(phi = c(0.7,-0.5), n=200, burnin=1000)
dl_sim4 <- arma_sim(phi = c(0.9,-0.4,0.2), n=200, burnin=1000)
dl_sim5 <- arma_sim(theta = 0.5, n=200, burnin=1000)
dl_sim6 <- arma_sim(phi=0.6, theta = 0.5, n=200, burnin=1000)
```
...
Nun sollen die Ergbnisse visualisiert werden. Dazu wird die simulierte Zeitreihe, die Vorhersagen und die Zeitreihe ohne Rauschen jeweils in einem Plot dargestellt.

```{r fig3, fig.width = 7, fig.asp = .3}

plot_dl_forecasts <- function(series, forecast){
        tbl <- tibble(Series=series$arma, Forecast=forecast, Lag=1:200)
        
        ggplot(tbl, aes(x=Lag)) + 
                geom_line(aes(y = Series), color = "darkred") + 
                geom_line(aes(y = Forecast), color="steelblue")
}

plot_dl_forecasts(dl_sim1, dl_1)
plot_dl_forecasts(dl_sim2, dl_2)
plot_dl_forecasts(dl_sim3, dl_3)
plot_dl_forecasts(dl_sim4, dl_4)
plot_dl_forecasts(dl_sim5, dl_5)
plot_dl_forecasts(dl_sim6, dl_6)
